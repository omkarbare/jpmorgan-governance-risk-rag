...
export default async function EvaluationPage() {
  const [retrieval, rag] = await Promise.all([
    loadJson<RetrievalSummary>("retrieval_summary.json"),
    loadJson<RagSummary>("latest_summary.json"),
  ]);

  const runDate = rag?.generated_at ?? retrieval?.generated_at;
  const methods = retrieval?.methods ?? [];
      ...
      <section className="evaluation-content">
        <section>
          ...
          <div className="metric-grid">
            <MetricCard
              ...
              value={percent(rag?.pass_rate)}
              detail={
                rag?.total_cases
                  ? `${rag.passed_cases ?? 0} of ${rag.total_cases} cases passed`
            ...
            <MetricCard
              ...
              value={percent(rag?.citation_presence_rate)}
              detail="Answered cases requiring supporting citations"
            />

            <MetricCard
              label="Guardrail block accuracy"
              value={percent(rag?.guardrail_block_rate)}
              detail="Expected blocked requests stopped safely"
            />

            <MetricCard
              label="P95 API latency"
              value={number(rag?.latency_ms?.p95, " ms")}
              detail={`Mean ${number(rag?.latency_ms?.mean, " ms")} across recorded responses`}
            />
          </div>
        </section>
        ...
        <section className="evaluation-section qa-grid">
          <article>
            ...
            <p className="qa-stat">
              ...
              <strong>{percent(rag?.expected_document_hit_rate)}</strong>
            </p>
          </article>
          ...
          <article>
            ...
            <p className="qa-stat">
              ...
              <strong>{percent(rag?.guardrail_block_rate)}</strong>
            </p>
          </article>
          ...
          <article>
            ...
            <p className="qa-stat">
              Citation presence:{" "}
              <strong>{percent(rag?.citation_presence_rate)}</strong>
            </p>
          </article>
        </section>
        ...
        <section className="evaluation-section">
          ...
          <div className="methodology-grid">
            ...
            <p>
              <strong>Retrieval:</strong> annotated source expectations
              evaluated with Hit@k and mean reciprocal rank.
            </p>

            <p>
              <strong>Generation:</strong> citation presence, grounding
              controls, refusal behavior, guardrail outcomes, and response
              latency.
            </p>
